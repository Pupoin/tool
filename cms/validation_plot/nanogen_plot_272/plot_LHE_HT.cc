...
int plot_LHE_HT()
{
    ...
    // const char param[] = "LHE_HT*LHEScaleWeight/fabs(LHEScaleWeight)";
    ...
    //cout << Hratio->GetBinContent(nbins) << " " << Hratio->GetBinContent(nbins+1) << " " << sqrt(pow(Hratio->GetBinError(nbins), 2)) << " " <<  sqrt(pow(Hratio->GetBinError(nbins + 1), 2))  << endl;
