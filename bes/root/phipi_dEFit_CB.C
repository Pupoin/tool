...
void phipi_dEFit_CB()
{
		...
		RooRealVar *sigma1=new RooRealVar("sigma1","width of gaussians",0.03,0.00,0.05) ;
    ...
    RooRealVar nsig2("ngaus","ngaus",10000,0,100000);
		...
		model->plotOn(xframe,Components(*cb),LineStyle(kDashed),LineColor(kRed));
		...
		TString Par9 = Form("#chi^{2}/ndf = %.0f/%.0d", curve->chiSquare(*histo,12)*(histo->GetN()-12),histo->GetN()-12);
