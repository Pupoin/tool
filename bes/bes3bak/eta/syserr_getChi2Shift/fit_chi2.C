...
int fit_chi2()
{
  ...
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/eta/data/cutmassroot/tightcut_r3c_minchi2/*data_removedouble.root");
  ...
  RooRealVar mean("mean", "mean", 9.9871e-02, -1, 1);
  RooRealVar sigma("sigma", "sigma", 3.9202e-02, 0, 0.5);
  ...
  chi2_min_r3c->setRange("fitrange", xmin, 50);
  ...
  sumpdf.plotOn(frame, Components("bkgPdf"),  LineStyle(2), LineWidth(6), LineColor(kGreen));
  ...
  // pt->SetLineColor(5);
  // pt->SetTextColor(5);
  TText *text = pt->AddText("#Lambda_{c}^{+} #rightarrow #Sigma^{+} #eta");
  pt->Draw();
  canv1->Update();
