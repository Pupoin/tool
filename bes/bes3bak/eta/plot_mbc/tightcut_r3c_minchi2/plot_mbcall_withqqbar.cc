...
using namespace std;
int plot_mbcall_withqqbar()
{

    // setStyle();
    // setPad();
    double factor[2] = {0.0170, 0.030};
    // double factor[2] = {0.0350, 0.042}; // nop
    ...
    TH1D *hist_qqbar = new TH1D("hist_qqbar", "m_{BC} Distribution", xbins, xmin, xmax);
    ...
    chain_hardon->Project("hist_hardon", "M_BC_r3c", cut_sum1 && "flag1<74100");
    chain_hardon->Project("hist_qqbar", "M_BC_r3c", cut_sum1 && "flag1>=74100");
    ...
    legend->AddEntry("hist_hardon", "hardon without  q#bar{q}", "f");
    ...
    canv1->Update();
    // canv1->SaveAs("etastackall_nopwithqqbar.pdf");
    // canv1->SaveAs("etastackall_nopwithqqbar.png");
    canv1->SaveAs("etastackall_withqqbar.pdf");
    canv1->SaveAs("etastackall_withqqbar.png");
    return 0;
}
