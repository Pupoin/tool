...
int plot_chi2all()
{
    ...
    chain_inclusive->Add("/publicfs/ucas/user/yuanchy8/7.0.6/eta/LL/cutmassroot/tightcut_r3c_minchi2/LL.root");
    chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/eta/data/cutmassroot/tightcut_r3c_minchi2/data.root");
    chain_hardon->Add("/publicfs/ucas/user/yuanchy8/7.0.6/eta/hadron/cutmassroot/tightcut_r3c_minchi2/hadron.root");
    ...
    chain_data->Project("hist_data", "chi2_min_r3c", cut_sum1);
    //TCut cut_mode = ("mode1==0 && mode2==0 && mode3==96");
    //chain_hardon->Project("hist_hardon", "chi2_min_r3c", cut_sum1 && "indexmc==0");
    chain_hardon->Project("hist_hardon", "chi2_min_r3c", cut_sum1);

    auto chain_inclusive_bkg = chain_inclusive;
    auto chain_inclusive_sig = chain_inclusive;
    chain_inclusive_bkg->Project("hist_inclusive_bkg", "chi2_min_r3c", cut_inclusive_bkg && cut_sum1);
    chain_inclusive_sig->Project("hist_inclusive_sig", "chi2_min_r3c", cut_inclusive_sig && cut_sum1);
    ...
    double fac = 1;
    cout <<  factor[0] /hist_data->GetEntries() << endl; 
    cout <<  factor[1] /hist_data->GetEntries() << endl; 
    cout <<  1/hist_data->GetEntries()  << endl; 

    hist_inclusive_sig->Scale(factor[0] /hist_data->GetEntries() );
    hist_inclusive_bkg->Scale(factor[0] /hist_data->GetEntries());
    hist_hardon->Scale(factor[1] /hist_data->GetEntries() );
    hist_data->Scale(1/hist_data->GetEntries() );

    cout << "LL bkg num *  weight :" ;
    cout << hist_inclusive_bkg->GetEntries()*factor[0] << endl;//707.586
    cout << "hadron bkg num *  weight :" ;
    cout << hist_hardon->GetEntries()*factor[1] << endl;    // 4236.89
    cout << hist_inclusive_bkg->GetEntries()*factor[0]/ hist_hardon->GetEntries()*factor[1] << endl; 
    ...
    hStack->GetXaxis()->SetTitle("#chi^{2}");
    ...
    canv1->Update();
    canv1->SaveAs("stackchi2all.pdf");
    canv1->SaveAs("stackchi2all.png");
    return 0;
}
