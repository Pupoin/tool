...
int fit_comb_loop()
{
    ...
    if(1)
    {
        ...
        for(int i = 0; i<7; i++)
        {
            ...
            frame_mode0_sig->SetTitle(TString::Format("#Lambda_{c}^{+} #rightarrow #Sigma^{+} #eta,     #sqrt{s}=%.3f GeV,     567 pb^{-1}", endpoint[i]*2 ));
            ...
            frame_mode0->SetTitle(TString::Format("#Lambda_{c}^{+} #rightarrow #Sigma^{+} #pi^{0},     #sqrt{s}=%.3f GeV,     567 pb^{-1}", endpoint[i]*2 ));
        ...
        // frame_mode0_sig->SetYTitle(TString::Format("Events/(%.2fMeV/c^{2})", (xmax - mbcmin) / xbins * 1000));
        // frame_mode0_sig->GetYaxis()->SetTitleOffset(1.2);
        // frame_mode0_sig->GetYaxis()->SetTitleSize(0.04);
        // frame_mode0_sig->GetXaxis()->SetTitleSize(0.04);
    ...
    if(1)
    {
        ...
        can3->SetFillColor(0);
        can3->SetMargin(0.15, 0.03, 0.15, 0.15);
        can3->cd();
        ...
        can4->SetFillColor(0);
        can4->SetMargin(0.15, 0.03, 0.15, 0.15);
        can4->cd();
