...
void fit_fit_comb()
{
   ...
   RooRealVar x("Br_Brnorm", "Br_Brnorm", -0.025, 0.025);
 

    // Construct gaussx(x,mx,1)
    RooRealVar mean("mean", "mean", 5.7821e-01, -0.001, 0.001);
    RooRealVar sigma("sigma", "sigma", 0.001, 0, 0.01);
    RooGaussian gauss("gauss", "gauss", x, mean, sigma);


    TChain *chain_data = new TChain("tree");
    chain_data->Add("Br_addBr.root");
    // --- Retrieve Data ---
   ...
   RooPlot *frame = x.frame(Title(" "), Bins(20));
   data->plotOn(frame);
   ...
   c->SaveAs("fit_fit_comb.png");
   c->SaveAs("fit_fit_comb.pdf");
