...
RooRealVar *M_BC_c = new RooRealVar("M_BC_c", "M_BC_c", 2.25, 2.34919, "");
...
int fitInclusive4600()
{
  ...
  TCut cut_mbc = "((M_BC_c>=2.25) && (M_BC_c<=2.299765))";
  TCut cut_sum1 = (cut_deltaE && cut_mbc && cut_np);
  

  // get the data _______________________________
  TChain *chain_data = new TChain("tree");
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/eta/signal/cutmassroot/tightcut_zhou2me/4600signal_removedouble.root");
  ...
  // chain_shape->Add("/publicfs/ucas/user/yuanchy8/7.0.6/eta/shape/cutmassroot/tightcut_zhou2me/4600shape.root");
  // TTree *tr_fit = chain_shape->CopyTree(cut_sum1, "");
  // RooRealVar *M_BC_c = new RooRealVar("M_BC_c", "M_BC_c", 2.25, 2.299765, "");
  ...
  TFile *file_sig_pdf = TFile::Open("/publicfs/ucas/user/yuanchy8/7.0.6/eta/pdf/tightcut_zhou2me/4600_sig_shapepdf.root");
  RooWorkspace *wspace = (RooWorkspace *)file_sig_pdf->Get("wspace");
  M_BC_c = wspace->var("M_BC_c");
  ...
  RooAbsPdf *SignalPdf = new RooFFTConvPdf("SignalPdf", "modelPdf (X) gauss", *M_BC_c, *modelPdf, gauss);
  ...
  RooPlot *frame = M_BC_c->frame(xmin, xmax, xbins);
  ...
  sumpdf.Print("t");
  M_BC_c->setRange("signal", 2.25, 2.299765);
  ...
  chain_datatruth->Add("/publicfs/ucas/user/yuanchy8/7.0.6/eta/signal/cutmassroot/tightcut_zhou2me/4600signal.root");
