...
int fit_chi2()
{
  ...
  TCut cut_ref = (cut_mbc && " (dipim<0.44 || dipim>0.52)" && cut_np && "chi2_min_r3c<50");
  // TCut cut_sig = (cut_mbc && "(chi2_min_r3c<100)" && cut_np);
  ...
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/pi0/data/cutmassroot/tightcut_r3c_minchi2/*data_adddipim.root");
  TTree *tree_cut = chain_data->CopyTree(cut_ref, "", 100000000, 0);
  ...
  TFile *file_LL_pdf = TFile::Open("LLbkgpdf.root");
  ...
  TFile *file_hadron_pdf = TFile::Open("hadronbkgpdf.root");
  ...
  RooRealVar frLL("frLL", "frLL", 517.35);
  RooRealVar frhardon("frhardon", "frhardon", 4236.89);
  ...
  RooRealVar nsig("nsig", "#sig events", 1.4758e+03, 0.01 * nentries, 1.2 * nentries);
  RooRealVar nbkg("nbkg", "#bkg events", 4.0281e+03, 0.0, 1.2 * nentries);
  // RooRealVar n_LLbkg("n_LLbkg", "#bkg events", 4.0281e+03, 0.0, 1.2 * nentries);
  // RooRealVar n_hadronbkg("n_hadronbkg", "#bkg events", 4.0281e+03, 0.0, 1.2 * nentries);
  ...
  // pt->SetLineColor(5);
  // pt->SetTextColor(5);
  TText *text = pt->AddText("#Lambda_{c}^{+} #rightarrow #Sigma^{+} #pi^{0}");
  pt->Draw();
  canv1->Update();
