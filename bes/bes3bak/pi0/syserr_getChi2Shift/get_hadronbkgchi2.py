...
cut_deltaE_ref = "(  np!=0 && npbar !=0 ) && (dipim<0.44 || dipim>0.52)"
cut_sig = (cut_mbc + " && "+  cut_deltaE_sig)
cut_ref = (cut_mbc  + " && "+  cut_deltaE_ref)
chi2_min_r3c = RooRealVar("chi2_min_r3c", "chi2_min_r3c", 0, 50)

can = TCanvas("can", "pdf", 800, 600) 
# can.Divide(2, 7)
LL_DirPath="/publicfs/ucas/user/yuanchy8/7.0.6/pi0/LL/cutmassroot/tightcut_r3c_minchi2/"
hadron_DirPath="/publicfs/ucas/user/yuanchy8/7.0.6/pi0/hadron/cutmassroot/tightcut_r3c_minchi2/"
#%% __ sig __

chain = TChain("tree")
chain.Add("/publicfs/ucas/user/yuanchy8/7.0.6/pi0/hadron/cutmassroot/tightcut_r3c_minchi2/hadron.root")
...
shapepdf = RooKeysPdf("modelPdf", "", chi2_min_r3c, shape, RooKeysPdf.MirrorBoth, 1.8)  
wspace = RooWorkspace("wspace", "wspace title")
# wspace.Import(shapepdf) 
getattr(wspace,'import')(shapepdf)
wspace.writeToFile("hadronbkgpdf.root")

frame = chi2_min_r3c.frame(ROOT.RooFit.Title("pi0 chi2 hadron "))
shape.plotOn(frame)
