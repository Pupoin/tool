...
for i in range(0, len(energy)):
    fff = LL_DirPath + "{0}LL_adddipim_rmsig.root".format(energy[i])
    print fff
    chain.Add(fff)
...
wspace.writeToFile("LLbkgpdf.root")

frame = chi2_min_r3c.frame(ROOT.RooFit.Title("pi0 chi2 LL "), ROOT.RooFit.Bins(20))
shape.plotOn(frame )
shapepdf.plotOn(frame, ROOT.RooFit.LineStyle(kDashed), ROOT.RooFit.LineColor(2)) 

can.cd()
frame.Draw()
can.SaveAs("LLbkgpdf.png")
