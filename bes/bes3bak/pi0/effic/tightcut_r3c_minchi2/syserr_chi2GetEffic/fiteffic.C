...
void singleFit(int index)
{
   ...
   RooRealVar x(name.c_str(), name.c_str(), -0.008, 0.006);

    // Construct gaussx(x,mx,1)
    RooRealVar mean("mean", "mean", 0, -0.005, 0.005);
   ...
   // pt->SetLineColor(5);
   // pt->SetTextColor(5);
   pt->InsertText(TString::Format("#Lambda_{c}^{+} #rightarrow #Sigma^{+} #pi^{0}, #sqrt{s}=%.3f GeV", endpoint[index]*2));
