...
do
    ...
    for i_e in ${energy[@]}
    do 
        sed -i "s|chain_data->Add.*$|chain_data->Add(\"${i_e}signal_adddipim_covGaus.root\");|" fitInclusive${i_e}.cc
    ...
    do  
        ...
        if [ "x${isSucess}" == "x1" ]
        then 
            effic="$(grep "efficiency" log.${i_e} | cut -d' ' -f3 )"
            echo "---0---- $i_e  ${isSucess}  $effic ------ "  
            echo "---0---- $i_e  ${isSucess}  $effic ------ " >> log.effic
            continue
        else
            tries=0
            while [ $tries -le 5 ]
            do 
                ...
                # sed -i "s|RooRealVar sigma.*$|RooRealVar sigma(\"sigma\", \"sigma\", $sigmav, 0, $sigmav+($sigmae*1.5));|" fitInclusive${i_e}.cc
                # sed -i "s|RooRealVar mean.*$|RooRealVar mean(\"mean\", \"mean\", $meanv, $meanv-($meane*1.5), $meanv+($meane*1.5));|" fitInclusive${i_e}.cc
                sed -i "s|RooRealVar sigma.*$|RooRealVar sigma(\"sigma\", \"sigma\", $sigmav, 0, $sigmav+($sigmae*10) );|" fitInclusive${i_e}.cc
                sed -i "s|RooRealVar mean.*$|RooRealVar mean(\"mean\", \"mean\", $meanv, $meanv-($meane*10), $meanv+($meane*10));|" fitInclusive${i_e}.cc
                # sed -i "s|RooRealVar sigma.*$|RooRealVar sigma(\"sigma\", \"sigma\", $sigmav, 0, $sigmav+($sigmae*1.5);|" fitInclusive${i_e}.cc
                # sed -i "s|RooRealVar mean.*$|RooRealVar mean(\"mean\", \"mean\", $meanv, $meanv-($meane*1.5), $meanv+($meane*1.5));|" fitInclusive${i_e}.cc
                root -l -b -q fitInclusive${i_e}.cc > log.${i_e} 
                ...
                then 
                    effic="$(grep "efficiency" log.${i_e} | cut -d' ' -f3 )"
                    echo "---$(( $tries + 1 ))---- $i_e  ${isSucess}  $effic ------ " 
                    echo "---$(( $tries + 1 ))---- $i_e  ${isSucess}  $effic ------ " >> log.effic
                    break
                fi 
                let tries++
            done
    ...
    /bin/rm *.root  log.4* *.png *.pdf fitIn*.cc
    cd ../../
    let n++    #或者写作n=$(( $n + 1 ))
done
