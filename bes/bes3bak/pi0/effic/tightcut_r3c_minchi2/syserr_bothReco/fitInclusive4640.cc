...
int fitInclusive4640()
{
  ...
  TCut cut_sum1 = (cut_chi2 && cut_chi2 && cut_np && "(dipim<0.44 || dipim>0.52)");
  ...
  m_fitres = sumpdf.fitTo(*data, Save(kTRUE), Extended(kTRUE), SumW2Error(true));
  m_fitres->Print("v");
  ...
  c1->SetMargin(0.13, 0.13, 0.13, 0.1);
  frame->Draw();
  ...
  // pt->SetLineColor(5);
  // pt->SetTextColor(5);
  pt->InsertText("#sqrt{s}=4641 GeV");
  pt->InsertText(TString::Format("effic = %.2f %%", ISIG/all*100.));
	pt->SetTextAlign(12);

  pt->Draw();
  c1->Update();
  c1->SaveAs("effic4641.pdf");
  c1->SaveAs("effic4641.png");
