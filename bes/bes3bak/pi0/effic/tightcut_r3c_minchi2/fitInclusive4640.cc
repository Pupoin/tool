...
RooRealVar *M_BC_r3c = new RooRealVar("M_BC_r3c", "M_BC_r3c", 2.25, 2.320255, "");

int fitInclusive4640()
{
  ...
  Double_t xmin = 2.25, xmax = 2.320255;
  Double_t   xbins = 50;
  TH1::SetDefaultSumw2(1);
  TString a("Events/");
  char b[20];
  sprintf(b, "(%5.1f", (xmax - xmin) / xbins * 1000);
  TString d(" MeV/#font[12]{c^{2}})");
  ...
  TCut cut_mbc = "((M_BC_r3c>=2.25) && (M_BC_r3c<=2.320255))";
  TCut cut_sum1 = (cut_chi2 && cut_mbc && cut_np && "(dipim<0.44 || dipim>0.52)");
  

  // get the data _______________________________
  TChain *chain_data = new TChain("tree");
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/pi0/signal/cutmassroot/tightcut_r3c_minchi2/4640signal_adddipim.root");
  ...
  // chain_shape->Add("../shape/cutmassroot/tightcut_r3c_minchi2/4640shape.root");
  ...
  TFile *file_sig_pdf = TFile::Open("/publicfs/ucas/user/yuanchy8/7.0.6/pi0/pdf/tightcut_r3c_minchi2/4640_ref_shapepdf.root");
  ...
  RooRealVar sigma("sigma", "sigma", 3.20842e-07, 0, 3.20842e-07+(1.50903e-09*5));
  RooRealVar mean("mean", "mean", 3.09731e-05, 3.09731e-05-(1.36508e-08*5), 3.09731e-05+(1.36508e-08*5));
  ...
  frame->GetXaxis()->SetTitle("M_{BC} (GeV/#font[12]{c^{2}})");
  ...
  sumpdf.Print("t");
  M_BC_r3c->setRange("signal",  2.25, 2.320255);
  ...
  chain_datatruth->Add("/publicfs/ucas/user/yuanchy8/7.0.6/pi0/signal/cutmassroot/tightcut_r3c_minchi2/4640signal.root");
  Double_t all = chain_datatruth->GetEntries();

  double truth = chain_data->GetEntries("(np !=0 && npbar != 0) && (chi2_min_r3c<17) && ((M_BC_r3c>=2.25) && (M_BC_r3c<=2.299765)) && (dipim<0.44 || dipim>0.52) ");
  ...
  TPaveText *pt = new TPaveText(0.16, 1-0.16-0.23, 0.43, 1-0.16, "blNDC");
  pt->SetBorderSize(0);
  pt->SetFillColor(0);
  // pt->SetLineColor(5);
  // pt->SetTextColor(5);
  pt->InsertText("#Lambda_{c}^{+} #rightarrow #Sigma^{+} #pi^{0}");
  pt->InsertText(TString::Format("#sqrt{s} = %.3f GeV", 2.320255*2) );
  pt->InsertText(TString::Format("#varepsilon^{ST} = %.2f #pm %.2f %%", ISIG/all*100., ISIGerr/all*100));
  // pt->InsertText(TString::Format("truth #varepsilon^{ST} = %.2f %%", truth/all*100.));
	pt->SetTextAlign(12);

  pt->Draw();
  canv1->Update();
  canv1->SaveAs("effic4640_Lc2pi0.pdf");
  canv1->SaveAs("effic4640.png");
  return 0;
}
