...
int plot_mbc4660()
{
    ...
    chain_inclusive->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/LL/cutmassroot/zhou2me/4660LL_mass.root");
    chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/data/cutmassroot/zhou2me/4660data_mass.root");
    chain_hardon->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/hadron/cutmassroot/zhou2me/4660hadron_mass.root");
    ...
    TCut cut_deltaE = "((deltaE_min>-0.033) && (deltaE_min<0.022))";
    // TCut cut_np = "(np!=0 && npbar != 0)";
    //TCut cut_chi2 = "(chi2_min_r1c<13)";
    TCut cut_mbc = "((M_BC>=2.25) && (M_BC<=2.3304600))";
    ...
    chain_hardon->Project("hist_hardon", "M_BC", cut_sum1 && !cut_mode);

    auto chain_inclusive_bkg = chain_inclusive;
    auto chain_inclusive_sig = chain_inclusive;
    chain_inclusive_bkg->Project("hist_inclusive_bkg", "M_BC", cut_inclusive_bkg && cut_sum1);
    chain_inclusive_sig->Project("hist_inclusive_sig", "M_BC", cut_inclusive_sig && cut_sum1);
