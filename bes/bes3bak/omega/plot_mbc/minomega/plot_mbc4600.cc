...
int plot_mbc4600()
{
    ...
    // setStyle();
    // setPad();
    double factor[2] = {0.03750, 0.0250};
    ...
    Int_t xbins = (xmax-xmin)*1000/2 + 1;
    xmax = xmin+xbins*0.002;
    ...
    chain_inclusive->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/LL/cutmassroot/minomega/4600LL_mass.root");
    chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/data/cutmassroot/minomega/4600data_mass.root");
    chain_hardon->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/hadron/cutmassroot/minomega/4600hadron_mass.root");
    ...
    TCut cut_deltaE = "((deltaE_min_1c>-0.023) && (deltaE_min_1c<0.015))";
    // TCut cut_np = "(np!=0 && npbar != 0)";
    //TCut cut_chi2 = "(chi2_min_r1c<13)";
    TCut cut_mbc = "((M_BC_1c>=2.25) && (M_BC_1c<=2.3))";
    TCut cut_sum1 = (cut_mbc && cut_deltaE);
    ...
    TCut cut_inclusive_sig = "(new_signal==1 || new_signal == -1)";
    TCut cut_inclusive_bkg = !cut_inclusive_sig;

    chain_data->Project("hist_data", "M_BC_1c", cut_sum1);
    TCut cut_mode = ("mode1==0 && mode2==0 && mode3==96");
    //chain_hardon->Project("hist_hardon", "M_BC_1c", cut_sum1 && "indexmc==0");
    chain_hardon->Project("hist_hardon", "M_BC_1c", cut_sum1 && !cut_mode);

    auto chain_inclusive_bkg = chain_inclusive;
    auto chain_inclusive_sig = chain_inclusive;
    chain_inclusive_bkg->Project("hist_inclusive_bkg", "M_BC_1c", cut_inclusive_bkg && cut_sum1);
    chain_inclusive_sig->Project("hist_inclusive_sig", "M_BC_1c", cut_inclusive_sig && cut_sum1);
    ...
    TLegend *legend = new TLegend(0.16, 0.6, 0.35, 0.85);
    legend->SetHeader("#sqrt{s}=4600 GeV");
    ...
    canv1->Update();
    canv1->SaveAs("stack4600.pdf");
    canv1->SaveAs("stack4600.png");
    return 0;
}
