...
int fitInclusive4600()
{
  ...
  TCut cut_mbc = "((M_BC_1c>=2.25) && (M_BC_1c<=2.299765))";
  TCut cut_sum1 = (cut_deltaE && cut_mbc);
  

  // get the data _______________________________
  TChain *chain_data = new TChain("tree");
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/signal/cutmassroot/minomega/4600signal_mass.root");
  ...
  // chain_shape->Add("../shape/cutmassroot/minomega/4600shape_mass.root");
  // TTree *tr_fit = chain_shape->CopyTree(cut_sum1, "");
  // RooRealVar *M_BC_1c = new RooRealVar("M_BC_1c", "M_BC_1c", 2.25, 2.299765, "");
  ...
  TFile *file_sig_pdf = TFile::Open("/publicfs/ucas/user/yuanchy8/7.0.6/omega/pdf/minomega/4600_ref_shapepdf.root");
  ...
  sumpdf.Print("t");
  M_BC_1c->setRange("signal", 2.25, 2.299765);
  ...
  chain_datatruth->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/signal/cutmassroot/minomega/4600signal.root");
