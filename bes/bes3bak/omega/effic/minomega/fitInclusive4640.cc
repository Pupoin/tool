...
int fitInclusive4640()
{
  ...
  TCut cut_mbc = "((M_BC_1c>=2.25) && (M_BC_1c<=2.320255))";
  TCut cut_sum1 = (cut_deltaE && cut_mbc);
  

  // get the data _______________________________
  TChain *chain_data = new TChain("tree");
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/signal/cutmassroot/minomega/4640signal_mass.root");
  ...
  // chain_shape->Add("../shape/cutmassroot/minomega/4640shape_mass.root");
  // TTree *tr_fit = chain_shape->CopyTree(cut_sum1, "");
  // RooRealVar *M_BC_1c = new RooRealVar("M_BC_1c", "M_BC_1c", 2.25, 2.320255, "");
  ...
  TFile *file_sig_pdf = TFile::Open("/publicfs/ucas/user/yuanchy8/7.0.6/omega/pdf/minomega/4640_ref_shapepdf.root");
  ...
  RooRealVar sigma("sigma", "sigma", 2.9389e-05, -0.003, 0.003);
  ...
  sumpdf.Print("t");
  M_BC_1c->setRange("signal", 2.25, 2.320255);
  ...
  chain_datatruth->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/signal/cutmassroot/minomega/4640signal.root");
