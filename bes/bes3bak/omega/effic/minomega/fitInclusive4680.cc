...
int fitInclusive4680()
{
  ...
  TCut cut_mbc = "((M_BC_1c>=2.25) && (M_BC_1c<=2.34079))";
  TCut cut_sum1 = (cut_deltaE && cut_mbc);
  

  // get the data _______________________________
  TChain *chain_data = new TChain("tree");
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/signal/cutmassroot/minomega/4680signal_mass.root");
  ...
  // chain_shape->Add("../shape/cutmassroot/minomega/4680shape_mass.root");
  // TTree *tr_fit = chain_shape->CopyTree(cut_sum1, "");
  // RooRealVar *M_BC_1c = new RooRealVar("M_BC_1c", "M_BC_1c", 2.25, 2.34079, "");
  ...
  TFile *file_sig_pdf = TFile::Open("/publicfs/ucas/user/yuanchy8/7.0.6/omega/pdf/minomega/4680_ref_shapepdf.root");
  ...
  RooRealVar sigma("sigma", "sigma", 0.0003076298, -0.00, 0.00081);
  RooRealVar mean("mean", "mean", -8.8463e-05, -0.001, 0.001);
  ...
  sumpdf.Print("t");
  M_BC_1c->setRange("signal", 2.25, 2.34079);
  ...
  chain_datatruth->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/signal/cutmassroot/minomega/4680signal.root");
