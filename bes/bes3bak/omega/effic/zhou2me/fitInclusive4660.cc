...
int fitInclusive4660()
{
  ...
  TCut cut_mbc = "((M_BC>=2.25) && (M_BC<=2.33046))";
  TCut cut_sum1 = (cut_deltaE && cut_mbc);
  

  // get the data _______________________________
  TChain *chain_data = new TChain("tree");
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/signal/cutmassroot/zhou2me/4660signal_mass.root");
  ...
  // chain_shape->Add("../shape/cutmassroot/zhou2me/4660shape_mass.root");
  // TTree *tr_fit = chain_shape->CopyTree(cut_sum1, "");
  // RooRealVar *M_BC = new RooRealVar("M_BC", "M_BC", 2.25, 2.33046, "");
  ...
  TFile *file_sig_pdf = TFile::Open("/publicfs/ucas/user/yuanchy8/7.0.6/omega/pdf/zhou2me/4660_ref_shapepdf.root");
  ...
  RooRealVar sigma("sigma", "sigma", 4.6706e-04, -0.0, 0.001);
  RooRealVar mean("mean", "mean", -2.4580e-05);//, -0.001, 0.001);
  ...
  sumpdf.Print("t");
  M_BC->setRange("signal", 2.25, 2.33046);
  ...
  chain_datatruth->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/signal/cutmassroot/zhou2me/4660signal.root");
