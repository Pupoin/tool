...
do
    ...
    for i_e in ${energy[@]}
    do 
        sed -i "s|chain_data->Add.*$|chain_data->Add(\"${i_e}signal_selB_covGaus.root\");|" fitInclusive${i_e}.cc
    ...
    do  
        ...
        # isSucess2=`grep -c "Full, accurate covariance matrix" log.${i_e}`
        # if [ "x${isSucess}" == "x1" ] && [ "x${isSucess2}" == "x1" ]
