...
int fit_omega()
{
  ...
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/omega/data/cutmassroot/tightcut_r3c_minomega/4*data.root");
  TTree *tree_cut = chain_data->CopyTree(cut_sum1, "", 10000000000, 0);
  ...
  // etaprimemr3c->setRange("signal", 0.72, 0.86);
  ...
  frame->GetXaxis()->SetTitle("m_{#omega} (GeV/#font[12]{c^{2}})");
  ...
  c1->SetMargin(0.15, 0.04, 0.15, 0.13);


    // setPad();

  frame->Draw();
  TPaveText *pt = new TPaveText(0.56, 0.62, 0.8, 0.8, "blNDC");
  pt->SetBorderSize(0);
  pt->SetFillColor(0);
  // pt->SetLineColor(5);
  // pt->SetTextColor(5);
  // pt->InsertText("#Lambda_{c}^{+} #rightarrow #Sigma^{+} #eta'");
  pt->InsertText(TString::Format("#mu  = %.2f #pm %.2f MeV/#font[12]{c^{2}}",  mean.getVal()*1000, mean.getError()*1000));
  pt->InsertText(TString::Format("#sigma = %.2f #pm %.2f MeV/#font[12]{c^{2}}", sigma.getVal()*1000, sigma.getError()*1000));
  ...
  c1->SaveAs("fit_omega_mass.png");
  c1->SaveAs("fit_omega_mass.pdf");
  return 0;
}
