void removedouble_mindE4620()
{
  TChain c("tree");
  c.Add("4620hadron_mode.root");

  int run, event, np, npbar, new_signal; double M_BC, deltaE_min, chi2_min_c;
  ...
  int run1, event1, np1, npbar1; double M_BC1, deltaE_min1, chi2_min_c1, new_signal1;
  int run2, event2, np2, npbar2; double M_BC2, deltaE_min2, chi2_min_c2, new_signal2;


  TFile f("4620hadron_removedouble_mode.root", "recreate");
  TTree tree("tree", "tree");
  int m_run, m_event, m_np, m_npbar; double m_M_BC, m_deltaE_min, m_chi2_min_c, m_new_signal;
