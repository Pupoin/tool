...
//   RooRealVar a3("a3", "a3", 0.01);
  RooPolynomial poly("pol3", "c + a1 * x ", x, RooArgSet(a1));
    ...
    chain_data->Add("4*_Costheta_sigma.root");
   ...
   //  TChain *chain_sd2 = new TChain("tree");
    chain_sd1->Add("4*_Costheta_sigma.root");
   ...
   //  chain_sd2->Project("histo_sideband2", "Costheta_sigma", "(M_BC_r3c>2.25 && M_BC_r3c<2.27) || (M_BC_r3c>2.31 && M_BC_r3c<2.33)");
    ...
    histo_sideband1->Scale(0.721341);
   //  histo_sideband2->Scale(0.62175);

    histo->Add(histo_sideband1, -1);
   ...
   TPaveText *pt = new TPaveText(0.16, 0.75, 0.45, 0.87, "blNDC");
   pt->SetBorderSize(0);
   pt->SetFillColor(0);
   // pt->SetLineColor(5);
   // pt->SetTextColor(5);
   pt->InsertText("#Lambda_{c}^{+} #rightarrow #Sigma^{+} #eta'");
   // pt->InsertText(TString::Format("%.3f #pm %.3f",  a1.getVal(), a1.getError()));
