...
void fiteffic()
{
   ...
   // for(int i=0; i<7 ; i++)
   {
      singleFit(0, -0.0015, 0.001, 10);
      singleFit(1, -0.0015, 0.0015, 10);
      singleFit(2, -0.001, 0.0006, 13);


      singleFit(3, -0.001, 0.0006, 15);
      singleFit(4, -0.001, 0.0006, 15);
      singleFit(5, -0.001, 0.0006, 15);
      singleFit(6, -0.001, 0.0006, 15);
...
void singleFit(int index, double x1=-0.0006, double x2=-0.00025, int xbin=15)
{
   ...
   RooRealVar x(name.c_str(), name.c_str(), x1, x2);
    ...
    RooRealVar mean("mean", "mean", -4.7821e-04, -0.002, 0.002);
   ...
   // pt->SetLineColor(5);
   // pt->SetTextColor(5);
   // pt->InsertText(TString::Format("#mu  = %.3e #pm %.1e",  mean.getVal(), sigma.getError()));
   // pt->InsertText(TString::Format("#sigma = %.3e #pm %.1e", sigma.getVal(), sigma.getError()));
