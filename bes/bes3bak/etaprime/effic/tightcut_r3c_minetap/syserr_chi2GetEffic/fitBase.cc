...
RooRealVar *M_BC_r3c = new RooRealVar("M_BC_r3c", "M_BC_r3c", 2.25, _endpoint_, "");

int fitInclusive_energy_()
{
  ...
  Double_t xmin = 2.25, xmax = _endpoint_;
  Double_t   xbins = 100;
  ...
  TCut cut_chi2 = "(chi2CovGaus<30)";
  TCut cut_mbc = "((M_BC_r3c>=2.25) && (M_BC_r3c<=_endpoint_))";
  TCut cut_sum1 = (cut_chi2 && cut_chi2 && cut_np);
  

  // get the data _______________________________
  TChain *chain_data = new TChain("tree");
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/signal/cutmassroot/tightcut_r3c_minetap/_energy_signal_selB.root");
  ...
  // chain_shape->Add("../shape/cutmassroot/tightcut_r3c_minetap/_energy_shape_mass.root");
  ...
  TFile *file_sig_pdf = TFile::Open("/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/pdf/tightcut_r3c_minetap/_energy__sig_shapepdf.root");
  ...
  RooRealVar sigma("sigma", "sigma", 4.0150e-04, -0.00000, 0.002);
  RooRealVar mean("mean", "mean",6.8704e-05, -0.001, 0.001);
  ...
  RooRealVar m0("m0", "m0", _endpoint_);
  RooRealVar p("p", "p", 1);
  ...
  sumpdf.Print("t");
  M_BC_r3c->setRange("signal", 2.25, _endpoint_);
  ...
  chain_datatruth->Add("/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/signal/cutmassroot/tightcut_r3c_minetap/_energy_signal.root");
  Double_t all = chain_datatruth->GetEntries();

  cout << TString::Format("efficiency = %.10f ", ISIG/all) << endl;
  ...
  // pt->SetLineColor(5);
  // pt->SetTextColor(5);
  pt->InsertText(TString::Format("#sqrt{s} = %.3f GeV", _endpoint_*2) );
  pt->InsertText(TString::Format("#varepsilon^{ST} = %.2f %%", ISIG/all*100.));
	pt->SetTextAlign(12);

  //pt->Draw();
  //canv1->Update();
  //canv1->SaveAs("effic_energy_.pdf");
  //canv1->SaveAs("effic_energy_.png");
