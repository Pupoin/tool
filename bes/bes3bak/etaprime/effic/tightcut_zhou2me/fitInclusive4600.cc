...
int fitInclusive4600()
{
  ...
  TCut cut_mbc = "((M_BC>=2.25) && (M_BC<=2.299765))";
  TCut cut_sum1 = (cut_deltaE && cut_mbc);
  

  // get the data _______________________________
  TChain *chain_data = new TChain("tree");
  chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/signal/cutmassroot/tightcut_zhou2me/4600signal_removedouble.root");
  // chain_data->Add("/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/LL/cutmassroot/tightcut_zhou2me/4600LL_removedouble.root");
  TTree *tree_cut = chain_data->CopyTree(cut_sum1, "", 100000000, 0);
  Int_t nentries = tree_cut->GetEntries();
  // --- Retrieve Data ---
  ...
  // chain_shape->Add("../shape/cutmassroot/tightcut_zhou2me/4600shape_mass.root");
  // TTree *tr_fit = chain_shape->CopyTree(cut_sum1, "");
  // RooRealVar *M_BC = new RooRealVar("M_BC", "M_BC", 2.25, 2.299765, "");
  ...
  TFile *file_sig_pdf = TFile::Open("/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/pdf/tightcut_zhou2me/4600_sig_shapepdf.root");
  ...
  RooRealVar sigma("sigma", "sigma", 1.4973e-05, -0.00,  0.001);
  RooRealVar mean("mean", "mean", -3.9582e-05, -0.001, 0.001);
  ...
  sumpdf.Print("t");
  M_BC->setRange("signal", 2.25, 2.299765);
  ...
  cout << " chain_data->GetEntries() "  << chain_data->GetEntries()  <<endl;
  cout << "m_fitres->minNll(): " <<m_fitres->minNll() << endl;


  TChain *chain_datatruth = new TChain("tree_truth");
  chain_datatruth->Add("/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/signal/cutmassroot/tightcut_zhou2me/4600signal_removedouble.root");
  Double_t all = chain_datatruth->GetEntries( );
  cout << " chain_datatruth->GetEntries((mode2==48 || mode3==48)) "  << all  <<endl;
