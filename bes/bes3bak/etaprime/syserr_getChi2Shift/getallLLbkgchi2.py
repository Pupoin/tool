...
# can.Divide(2, 7)
LL_DirPath="/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/LL/cutmassroot/tightcut_r3c_minetap/LL_split_rmsig.root"
hadron_DirPath="/publicfs/ucas/user/yuanchy8/7.0.6/etaprime/hadron/cutmassroot/tightcut_r3c_minetap/hadron.root"
#%% __ sig __

chain = TChain("tree")
chain.Add(LL_DirPath)

# for i in range(0, len(energy)):
#     fff = LL_DirPath + "{0}LL_rmsig.root".format(energy[i])
#     print fff
#     chain.Add(fff)
...
shapepdf = RooKeysPdf("modelPdf", "", chi2_min_r3c, shape, RooKeysPdf.MirrorRight, 3)  
wspace = RooWorkspace("wspace", "wspace title")
# wspace.Import(shapepdf) 
getattr(wspace,'import')(shapepdf)
wspace.writeToFile("allchi2_LLbkgpdf.root")

frame = chi2_min_r3c.frame(ROOT.RooFit.Title("eta chi2 LL "), ROOT.RooFit.Bins(20))
shape.plotOn(frame)
shapepdf.plotOn(frame, ROOT.RooFit.LineStyle(kDashed), ROOT.RooFit.LineColor(2)) 

can.cd()
frame.Draw()
can.SaveAs("allchi2_LLbkgpdf.png")
