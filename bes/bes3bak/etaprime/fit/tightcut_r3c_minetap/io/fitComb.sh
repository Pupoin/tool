...
do
    ...
    for(( i=0;i<10;i++)) 
    do 
        if [ $n -eq 2 ] || [ $n -eq 3 ] || [ $n -eq 6 ] || [ $n -eq 8 ] || [ $n -eq 9 ] || [ $n -eq 10 ] || [ $n -eq 11 ] || [ $n -eq 12 ] || [ $n -eq 13 ] || [ $n -eq 14 ] || [ $n -eq 15 ] || [ $n -eq 16 ] || [ $n -eq 17 ] || [ $n -eq 19 ] || [ $n -eq 20 ] || [ $n -eq 21 ] || [ $n -eq 22 ] || [ $n -eq 23 ] || [ $n -eq 24 ] || [ $n -eq 26 ] || [ $n -eq 27 ] || [ $n -eq 28 ] || [ $n -eq 29 ] || [ $n -eq 30 ] || [ $n -eq 31 ] || [ $n -eq 35 ] || [ $n -eq 36 ] || [ $n -eq 37 ] || [ $n -eq 38 ]
        then
            let n++
            continue 
        fi 
        ...
        sed "s|io.png|io_${n}.png|g" -i "${fileName}.C"
    ...
    do  
        isSucess=`grep -c "Status : MIGRAD=0 HESSE=0" log.${mName} `
        if [ "x${isSucess}" == "x1" ]
        then 
    ...
    echo "parallel 7 end, start a new one "
#     cd ../../
#     let n++    #或者写作n=$(( $n + 1 ))
done
