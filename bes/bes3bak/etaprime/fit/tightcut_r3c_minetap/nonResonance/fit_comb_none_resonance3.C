...
TCut cut_ref = (cut_mbc && "(chi2_min_r3c< 30)" && cut_np && "((etaprimemr3c>0.68&&etaprimemr3c<0.72)||(etaprimemr3c>0.84&&etaprimemr3c<0.89))");
// TCut cut_sig = (cut_mbc && "(chi2_min_r3c< 30)" && cut_np && "((etaprimemr3c>0.91&&etaprimemr3c<0.93)||(etaprimemr3c>0.98&&etaprimemr3c<1.))");
...
RooRealVar Bratio("Bratio", "Br of sig/ref", 0., 0., 100);	
// RooRealVar Bratio("Bratio", "Br of sig/ref", 0);	

int fit_comb_none_resonance3()
{
    ...
    RooRealVar mean_all("mean_all", "mean_all", -1.0572e-04, -0.0001, 0.0001);
    RooRealVar sigma_all("sigma_all", "sigma_all",  3.5676e-06, 0, 0.00001);
    ...
    allList.add(*ref_nl_4700);
        ...
        // model0->plotOn(frame_mode0, Components("*sigpdf0"), LineColor(kGreen+2), LineWidth(3), LineStyle(kDashed));
        // frame_mode0->Draw();
        // return 0;
    ...
    if(1)
    {
        ...
        for(int i = 0; i<7; i++)
        {
            ...
            mode_sig[i]->plotOn(frame_mode0_sig, Components(TString::Format("sigpdf%1d_sig", i)), LineColor(kGreen+2), LineWidth(3), LineStyle(kDashed));
            ...
            mode_ref[i]->plotOn(frame_mode0, Components(TString::Format("sigpdf%1d", i)), LineColor(kGreen+2), LineWidth(3), LineStyle(kDashed));
    ...
    if(1)
    {
        ...
        pdfSig->plotOn(frame_mode_sig, Components("sigpdf*_sig"), LineColor(kGreen+2), LineWidth(3), LineStyle(kDashed));
        ...
        can3->SaveAs("sideband_etapfit.png");
        can3->SaveAs("sideband_etapfit.pdf");
        ...
        pdfRef->plotOn(frame_mode_ref, Components("sigpdf*"), LineColor(kGreen+2), LineWidth(3), LineStyle(kDashed));
        ...
        can4->SaveAs("sideband_omegafit.png");
        can4->SaveAs("sideband_omegafit.pdf");
    }
