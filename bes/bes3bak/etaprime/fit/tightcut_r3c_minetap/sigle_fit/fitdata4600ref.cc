...
int fitdata4600ref()
{
  ...
  chain_data->Add(("/publicfs/ucas/user/yuanchy8/7.0.6/omega/data/cutmassroot/tightcut_r3c_minomega/" + energy[myindex] + "data_selB.root").c_str());
  ...
  TFile *file_sig_pdf = TFile::Open(("/publicfs/ucas/user/yuanchy8/7.0.6/omega/pdf/tightcut_r3c_minomega/"+ energy[myindex] + "_ref_shapepdf.root").c_str());
  ...
  RooRealVar mean("mean", "mean", 1.5469e-04, -0.001, 0.001);
  ...
  // pt->SetLineColor(5);
  // pt->SetTextColor(5);
  TText *text = pt->AddText(("E=" + energy[myindex] + "GeV").c_str());
  pt->Draw();
  c1->Update();
  ...
  c1->SaveAs("pic_fitInclusive4600_ref.png");
  return 0;
}
